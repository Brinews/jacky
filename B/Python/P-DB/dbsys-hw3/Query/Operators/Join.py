...
class Join(Operator):
	...
	def localCost(self, estimated):
		l = self.inputs()[0].cardinality(estimated)
		r = self.inputs()[1].cardinality(estimated)
		lpcap = 0
		for (lPageId, lhsPage) in self.lhsPlan:
			lpcap = lhsPage.header.pageCapacity / lhsPage.header.tupleSize
			break;
			
		rpcap = 0
		for (rPageId, rhsPage) in self.rhsPlan:
			rpcap = rhsPage.header.pageCapacity / rhsPage.header.tupleSize
			break;
			
		if self.joinMethod == "nested-loops":
			return (r + rcap * l) * self.tupleCost
			
		elif self.joinMethod == "block-nested-loops":
			return (r + r / ((self.buffFree - 2) * l)) * self.tupleCost
