...
public class TupleDesc {
    ...
    /**
     * Returns a list of the column names in the schema
     */
    ...
    /**
     * Generates a new schema based on a join between two schemas
     */
    public static TupleDesc join(TupleDesc left, TupleDesc right) {
        TupleDesc joined = new TupleDesc();
        for(TupleDescItem item : left.columns) {
            joined.columns.add(item);
        }
        for(TupleDescItem item : right.columns) {
            if(!joined.hasField(item.getName())) {
                joined.columns.add(item);
            }
        }
        return joined;
