...
static TreeNode*  array_dcl(RDinfo * info, Bool *  ok){
	...
	TokenNode *n4 = reach_node(info->thisTokenNode, 4);
	TokenNode *n5 = reach_node(info->thisTokenNode, 5);
	...
	if (check(info->thisTokenNode, VOID) || check(info->thisTokenNode, INT)) {
		if (n != NULL && check(n, ID)
			...
			node->attr.dclAttr.type = (type == VOID ? VOID_TYPE : INT_TYPE);
			node->attr.dclAttr.size = atoi(tokenString(n3));
...
static TreeNode * fun_dcl(RDinfo * info, Bool *  ok){
	...
	if (type != END && match_move(info, ID, "fun-dcl: need 'ID")) {
			...
			node->attr.dclAttr.type = (type == VOID ? VOID_TYPE : INT_TYPE);
			node->attr.dclAttr.name = string_clone(tokenString(id));
...
TreeNode * params(RDinfo * info, Bool *  ok){
	TokenNode *n1 = reach_node(info->thisTokenNode, 0);
...
TreeNode * param(RDinfo * info, Bool *  ok){
	...
	if (type != END) {
			...
			if (n1 != NULL && check(n1, ID)) {
				...
				node->attr.dclAttr.name = string_clone(tokenString(n1));
	...
	if (check(info->thisTokenNode, ID)) {
		if (match_move(info, ID, "array_element() : need ID"))
		   if (match_move(info, LBR, "array_element() : need LBR"))
			  if (node = expression(info, &status), status) {
				  TreeNode *root = new_expr_node(OP_EXPR, info->thisTokenNode->lineNum);
				  if (match_move(info, RBR, "array_element() : need RBR"))
					root->attr.exprAttr.op  = LBR;

					root->child[0] = node;

					return parse_good_return(root, ok);
			  }
		...
		if (match_move(info, LPAR, "array_element(): need LPAR")) 
			if (node = expression(info, &status), status)
				if (match_move(info, RPAR, "array_element(): need RPAR"))
					if (match_move(info, LBR, "array_element(): need LBR"))
					   if (node1 = expression(info, &status), status) {
						   TreeNode *root = new_expr_node(OP_EXPR, info->thisTokenNode->lineNum);
						   if (match_move(info, RBR, "array_element(): need RBR")) 
...
TreeNode * relational_expr(RDinfo * info, Bool *  ok){
	...
	if (check(info->thisTokenNode, LT) || check(info->thisTokenNode, LTE)
					|| check(info->thisTokenNode, GT) || check(info->thisTokenNode, GTE)) {
...
static TreeNode * call(RDinfo * info, Bool *  ok){
	...
	if (match_move(info, ID, "call() : need ID"))
		if (match_move(info, LPAR, "call(): need LPAR")) {
			...
			node->attr.exprAttr.name = string_clone(tokenString(n0));
