...
static const char* tokenContent(TokenNode *tn)
{
	return tn->token->string;
}

#define PARSERINFO() do { \
	if(RD_PARSE_DEBUG){ \
...
static TreeNode * var_dcl(RDinfo * info, Bool *  ok){
	...
	TokenNode *snapshot = info->thisTokenNode;

	if (check(info->thisTokenNode, VOID) || check(info->thisTokenNode, INT)) {
			if (check(reach_node(info->thisTokenNode, 1), ID)
				&& check(reach_node(info->thisTokenNode, 2), SEMI)) {

				PARSERINFO();

				TokenType type = type_specifier(info); /* already move next one token */
				info->thisTokenNode = reach_node(snapshot, 3);

				node = new_dcl_node(VAR_DCL, lineNum);
				node->attr.dclAttr.type = (type == VOID ? VOID_TYPE : INT_TYPE);

				if (type != VOID) {
					node->attr.dclAttr.name = string_clone(tokenContent(reach_node(snapshot, 1)));
				}
...
static TreeNode*  array_dcl(RDinfo * info, Bool *  ok){
	...
	TokenNode *snapshot = info->thisTokenNode;
	TokenNode *n1 = reach_node(info->thisTokenNode, 1);
	TokenNode *n2 = reach_node(n1, 1);
	TokenNode *n3 = reach_node(n2, 1);
	TokenNode *n4 = reach_node(n3, 1);
	TokenNode *n5 = reach_node(n4, 1);

	if (check(info->thisTokenNode, VOID) || check(info->thisTokenNode, INT)) {
		if (check(n1, ID) && check(n2,LBR) && check(n3, NUM) && check(n4, RBR) && check(n5, SEMI)) {

			PARSERINFO();

			TokenType type = type_specifier(info); /* already move next one token */

			info->thisTokenNode = reach_node(snapshot, 6);

			node = new_dcl_node(ARRAY_DCL, lineNum);

			node->attr.dclAttr.type = (type == VOID ? VOID_TYPE : INT_TYPE);
			node->attr.dclAttr.size = atoi(tokenContent(n3));
			node->attr.dclAttr.name = string_clone(tokenContent(n1));
...
static TreeNode * fun_dcl(RDinfo * info, Bool *  ok){
	...
	if (type != END && match_move(info, ID, "fun-dcl: need 'ID")) {
		...
		node->attr.dclAttr.type = (type == VOID ? VOID_TYPE : INT_TYPE);
		node->attr.dclAttr.name = string_clone(tokenContent(id));

		if (match_move(info, LPAR, "fun-dcl: need LPAR") && (node->child[0] = params(info, &status), status))
			if (match_move(info, RPAR, "fun-dcl: nedd RPAR") && (node->child[1] = compound_stmt(info, &status), status))
				return parse_good_return(node, ok);
	}
...
TreeNode * params(RDinfo * info, Bool *  ok){
	TokenNode *n1 = info->thisTokenNode;
	TokenNode *n2 = reach_node(info->thisTokenNode, 1);

	if (check(n1, VOID) && check(n2, RPAR)) {
...
TreeNode * param(RDinfo * info, Bool *  ok){
	...
	TokenNode *n1 = reach_node(info->thisTokenNode, 1);
	TokenNode *n2 = reach_node(info->thisTokenNode, 2);
	TokenNode *n3 = reach_node(info->thisTokenNode, 3);
	...
	if (type != END) {
			...
			if (check(n1, ID)) {
				...
				info->thisTokenNode = reach_node(info->thisTokenNode, steps);
...
TreeNode * compound_stmt(RDinfo * info, Bool *  ok){
	...
	if (match_move(info, LCUR, "compound-stmt(): need LCUR") && (left = local_dcl_list(info, &status), status))
			if (right = stmt_list(info, &status), status)
...
static TreeNode * local_dcl(RDinfo * info, Bool *  ok){
	...
	if (node != NULL) return parse_good_return(node, ok); 
...
TreeNode * selection_stmt(RDinfo * info, Bool *  ok){
	...
	if (match_move(info, IF, "selection_stmt(): need IF") && match_move(info, LPAR, "selection_stmt(): need LPAR")) {
		...
		if (node->child[0] = expression(info, &status), status)
			if (match_move(info, RPAR, "selection_stmt(): need RPAR") && (node->child[1] = statement(info, &status), status))
...
static TreeNode * while_stmt(RDinfo * info, Bool *  ok){
	...
	if(match_move(info, WHILE, "while_stmt(): need WHILE") && match_move(info, LPAR, "while_stmt(): need LPAR")) {
		...
		if(nd->child[0] = expression(info, &status), status)
			if(match_move(info, RPAR, "while_stmt(): need RPAR") && (nd->child[1] = statement(info, &status), status))
...
static TreeNode * for_stmt(RDinfo * info, Bool *  ok){
	...
	if (match_move(info, FOR, "fo_stmt(): need FOR") && match_move(info, LPAR, "for_stmt(): need LPAR")) {
		...
		if(node->child[0] = expr_or_empty(info, &status), status)
			if (match_move(info, SEMI, "for_stmt(): need SEMI") && (node->child[1] = expr_or_empty(info, &status), status))
			    if (match_move(info, SEMI, "for_stmt(): need SEMI") && (node->child[2] = expr_or_empty(info, &status), status))
				  if (match_move(info, RPAR, "for_stmt(): need RPAR") && (node->child[3] = statement(info, &status), status))
				     return parse_good_return(node, ok);
	}
...
TreeNode * return_stmt(RDinfo * info, Bool *  ok){
	...
	if (match_move(info, RETURN, "return_stmt(): need RETURN")) {
		...
		if((node->child[0] = expr_or_empty(info, &status), status) && match_move(info, SEMI, "return_stmt(): need SEMI"))
				return parse_good_return(node, ok);
	}
...
static TreeNode * expr_stmt(RDinfo * info, Bool *  ok){
	...
	PARSERINFO();
	if ((node->child[0]=expression(info, &status), status) && match_move(info, SEMI, "expr_stmt(): need SEMI"))
		  return parse_good_return(node, ok);
...
static TreeNode * comma_expr(RDinfo * info, Bool *  ok){
	...
	while (!(check(info->thisTokenNode, RPAR) || check(info->thisTokenNode, SEMI) || check(info->thisTokenNode, RBR))) {

		if (right != NULL) {
			if (!match_move(info, COMMA, "comma_expr() : need COMMA")) return parse_bad_return(first, ok);
		}
...
static TreeNode * lhs(RDinfo * info, Bool *  ok){
	...
	TokenNode *n0 = info->thisTokenNode;
	TokenNode *n1 = reach_node(info->thisTokenNode, 1);

	if (check(n1, LBR) || check(n0, LPAR)) {
		node = array_element(info, ok);
		if (*ok == TRUE) return parse_good_return(node, ok);
	}
	else if (check(info->thisTokenNode, ID) && match_move(info, ID, "lhs(): need ID")) {
		node = new_expr_node(ID_EXPR, n0->lineNum);
		node->attr.exprAttr.name  = string_clone(tokenContent(n0));
...
static TreeNode * array_element(RDinfo * info, Bool *  ok){
	...
	if (check(info->thisTokenNode, ID)) {
		if (match_move(info, ID, "array_element() : need ID") && match_move(info, LBR, "array_element() : need LBR"))
			  if (node = expression(info, &status), status) {
				  TreeNode *rootNode = new_expr_node(OP_EXPR, info->thisTokenNode->lineNum);
		...
		if (match_move(info, LPAR, "array_element(): need LPAR")) 
			if (node = expression(info, &status), status)
				if (match_move(info, RPAR, "array_element(): need RPAR") && match_move(info, LBR, "array_element(): need LBR"))
					   if (right = expression(info, &status), status) {
						   TreeNode *rootNode = new_expr_node(OP_EXPR, info->thisTokenNode->lineNum);
						   if (match_move(info, RBR, "array_element(): need RBR")) 

							  rootNode->attr.exprAttr.op  = LBR;
							  rootNode->child[0] = node;
							  rootNode->child[1] = right;
...
TreeNode * multiplicative_expr(RDinfo * info, Bool *  ok){
	...
	if (check(info->thisTokenNode, TIMES) || check(info->thisTokenNode, OVER)) {
			...
			rootNode = mulop(info, ok);
			rootNode->child[0] = node;
			node = rootNode;
			info->thisTokenNode = reach_node(info->thisTokenNode, 1);
			if (node != NULL) node->child[1] = multiplicative_expr(info, ok);
	}
...
TreeNode * primary_expr(RDinfo * info, Bool *  ok) {
	...
	if (check(info->thisTokenNode, ID)) {
			...
			else if (check(n1, LPAR)) {
				node = call(info, &status);
			}
			else {
				node = new_expr_node(ID_EXPR, info->thisTokenNode->lineNum);
				node->attr.exprAttr.name = string_clone(tokenContent(info->thisTokenNode));
				status = TRUE;
			...
			node->attr.exprAttr.val = atoi(tokenContent(info->thisTokenNode));
			...
			if (match_move(info, LPAR, "primary_expr(): need LPAR") && (node = expression(info, &status), status)) 
					if (match_move(info, RPAR, "primary_expr(): need RPAR")) {
						if (check(info->thisTokenNode, LBR)){
							info->thisTokenNode = n0;
							/* array parse */
							if (node = array_element(info, &status), status)
								return parse_good_return(node, ok);
							else
								return parse_bad_return(node, ok);
						}
...
static TreeNode * call(RDinfo * info, Bool *  ok){
	...
	if (match_move(info, ID, "call() : need ID") && match_move(info, LPAR, "call(): need LPAR")) {
		...
		if ((node->child[0] = arg_list(info, &status), status) && match_move(info, RPAR, "call(): need RPAR"))
			return parse_good_return(node, ok);
	}
...
static TreeNode * arg_list(RDinfo * info, Bool *  ok){
	...
	while (!check(info->thisTokenNode, RPAR)) {
		...
		if (right != NULL) {
			if (!match_move(info, COMMA, "arg_list(): need COMMA")) return parse_bad_return(first, ok);
		}
		right = assignment_expr(info, &status);
