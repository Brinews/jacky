...
Bool RD_PARSE_DEBUG = TRUE;  /* by default as false, do not print debug information */
...
/* The syntax grammar of C-
   ...
   variable ===> productiocurrentNode | productiosecondNode | ...
 */
 ...
 *   each function do not need to check if thisTokenNode==NULL, since the END token should be reached first, which should indicate
 *   the end of parsing, success or failure. However, be careful to expression like thisTokenNode->next->next. Make sure that
 ...
 *   	-  The value of thisTokenNode is not specified. It points to some node in the tokenList, not earlier then the place where this function call starts.
 ...
 *   and consumes thisTokenNode (move to the next token node). If not, it returns END.
...
static TreeNode * dcl_list(RDinfo * info, Bool *  ok){
	...
	while(!check(info->thisTokenNode, END)){
		rightNode = declaration(info, &status);
		if(lastNode == NULL){ // when the first node of declaration list is obtained.
			firstNode = lastNode = rightNode;
		}
		else if(rightNode!=NULL){
			lastNode  ->rSibling = rightNode;
			rightNode ->lSibling = lastNode;
			lastNode = rightNode;
		}
		if(status==FALSE)
			return parse_bad_return(firstNode, ok);
	}
	return parse_good_return(firstNode, ok); /* success */
}
 ...
 */
static TreeNode * declaration(RDinfo * info, Bool *  ok){
	TokenNode * thisTokenNode = info->thisTokenNode;
	TreeNode *node = NULL;
	*ok = FALSE;

	if (RD_PARSE_DEBUG) {
		printf( "%20s  %-10d", __FUNCTION__, thisTokenNode->lineNum);  print_token(thisTokenNode->token); printf( "\n");
	}

	node = var_dcl(info, ok); if (node != NULL) return node;

	node = array_dcl(info, ok); if (node != NULL) return node;

	node = fun_dcl(info, ok); if (node != NULL) return node;
...
static TokenType type_specifier(RDinfo * info){
	...
	syntax_error(info, thisTokenNode, "type_specifier(): need to have int or void");
	return END;
}
...
/* 5  var-dcl  ===>  type-specifier 'ID '; 
   ...
   - the type of its declaration-attributes is INT_TYPE or VOID_TYPE according to the token type of thisTokenNode.
...
static TreeNode * var_dcl(RDinfo * info, Bool *  ok){
	...
	TokenNode *originNode = thisTokenNode;

	TokenNode *n  = reach_node(thisTokenNode, 1);
	TokenNode *secondNode = reach_node(thisTokenNode, 2);

	if (check(thisTokenNode, VOID) || check(thisTokenNode, INT)) {
		if (NULL != n && check(n, ID)
				&& NULL != secondNode && check(secondNode, SEMI)) {
			...
			info->thisTokenNode = reach_node(originNode, 3);

			node = new_dcl_node(VAR_DCL, lineNum);
			node->attr.dclAttr.type = (type == VOID ? VOID_TYPE : INT_TYPE);
			node->attr.dclAttr.name = string_clone(getTokenStr(n));
...
static TreeNode*  array_dcl(RDinfo * info, Bool *  ok){
	...
	TokenNode *n  = reach_node(thisTokenNode, 1);
	TokenNode *secondNode = reach_node(thisTokenNode, 2);
	TokenNode *nextNode = reach_node(thisTokenNode, 3);
	TokenNode *thirdNode = reach_node(thisTokenNode, 4);
	TokenNode *fourthNode = reach_node(thisTokenNode, 5);

	if (check(thisTokenNode, VOID) || check(thisTokenNode, INT)) {
		if (n != NULL && check(n, ID)
				&& secondNode != NULL && check(secondNode,LBR)
				&& nextNode != NULL && check(nextNode, NUM)
				&& thirdNode != NULL && check(thirdNode, RBR)
				&& fourthNode != NULL && check(fourthNode, SEMI)) {
			...
			info->thisTokenNode = reach_node(originNode, 6);

			node = new_dcl_node(ARRAY_DCL, lineNum);
			node->attr.dclAttr.type = (type == VOID ? VOID_TYPE : INT_TYPE);
			node->attr.dclAttr.size = atoi(getTokenStr(nextNode));
			node->attr.dclAttr.name = string_clone(getTokenStr(n));
...
static TreeNode * fun_dcl(RDinfo * info, Bool *  ok){
	...
	TokenType type = type_specifier(info); /* already move next one token */
	TokenNode *id = info->thisTokenNode;

	if (type != END && match_move(info,ID, "fun-dcl: need 'ID")) {

		node->attr.dclAttr.type = (type == VOID ? VOID_TYPE : INT_TYPE);
		node->attr.dclAttr.name = string_clone(getTokenStr(id));
...
TreeNode * params(RDinfo * info, Bool *  ok){
	...
	TokenNode *currentNode = reach_node(thisTokenNode, 0);
	TokenNode *secondNode = reach_node(thisTokenNode, 1);

	if(RD_PARSE_DEBUG) {
		printf("%20s  %-10d", __FUNCTION__, thisTokenNode->lineNum);  print_token(thisTokenNode->token); printf("\n");
	}

	if (currentNode != NULL && check(currentNode, VOID)
			&& secondNode != NULL && check(secondNode, RPAR)) {

		TreeNode *node = new_param_node(VOID_PARAM, currentNode->lineNum);

		node->attr.dclAttr.type = VOID_TYPE;

		info->thisTokenNode = secondNode;
...
static TreeNode * param_list(RDinfo * info, Bool *  ok){
	...
	while (!check(info->thisTokenNode, RPAR)) {
		...
		right = param(info, &status);

		if (last == NULL) { first = last = right; }
		else if (right != NULL) {
			last->rSibling = right;
			right->lSibling = last;
...
TreeNode * param(RDinfo * info, Bool *  ok){
	...
	TokenNode *currentNode = reach_node(thisTokenNode, 1);
	TokenNode *secondNode = reach_node(thisTokenNode, 2);
	TokenNode *nextNode = reach_node(thisTokenNode, 3);

	TokenType type = type_specifier(info); /* already move next one token */
	thisTokenNode = info->thisTokenNode;
	...
	if (type != END) {
		...
		if (currentNode != NULL && check(currentNode, ID)) {
			steps = 1;

			if (secondNode != NULL && check(secondNode, LBR)
					&& nextNode != NULL && check(nextNode, RBR)) {
				pkind = ARRAY_PARAM;
				steps += 2;
			}

			node = new_param_node(pkind, currentNode->lineNum);
			node->attr.dclAttr.type = (type == INT ? INT_TYPE : VOID_TYPE);
			node->attr.dclAttr.name = string_clone(getTokenStr(currentNode));

			info->thisTokenNode = reach_node(thisTokenNode, steps);
...
/* 12   local-dcl-list  ===>  local-dcl local-dcl-list | EMPTY
   ...
   - NULL if thisTokenNode is not INT nor VOID
...
/* 13   local-dcl		===>  var-dcl | array-dcl
 * If thisTokenNode is ID followed by [, returns array_dcl(),
...
static TreeNode * local_dcl(RDinfo * info, Bool *  ok){
	...
	node = var_dcl(info, ok);
	if (node != NULL) {
		*ok = TRUE;
		return node;
	}

	node = array_dcl(info, ok);
	if (node != NULL) {
		*ok = TRUE;
		return node;
	}
...
TreeNode * stmt_list(RDinfo * info, Bool *  ok){
	...
	while (!check(info->thisTokenNode, RCUR)) {
		rightNode = statement(info, &status);
	...
	return parse_good_return(firstNode, ok);
}

/*15
 statement 		===>  compound-stmt | selection-stmt | while-stmt | do-while-stmt | for-stmt | return-stmt | null-stmt | expr-stmt
 Returns the return of a call to the corresponding function, depending on thisTokenNode:
...
TreeNode * selection_stmt(RDinfo * info, Bool *  ok){
	...
	if (match_move(info,IF, "selection_stmt(): need IF"))
		if (match_move(info,LPAR, "selection_stmt(): need LPAR"))
...
/*
 17. else-part		===>  'ELSE statement | EMPTY
  If thisTokenNode is ELSE, returns statement(),
...
static TreeNode * while_stmt(RDinfo * info, Bool *  ok){
	...
	if(match_move (info, WHILE, "while_stmt(): need WHILE"))
		if(match_move (info, LPAR, "while_stmt(): need LPAR"))
...
static TreeNode * do_while_stmt(RDinfo * info, Bool *  ok){
	...
	if (match_move(info,DO, "do_while_stmt(): need DO")) 
		if(node->child[0] = statement(info, &status), status) {
			if (match_move(info,WHILE, "do_while_stmt(): need WHILE"))
			  if (match_move(info,LPAR, "do_while_stmt(): need LPAR")) 
				  if (node->child[1] = expression(info, &status), status)
					  if (match_move(info,RPAR, "do_while_stmt(): need RPAR"))
					    if (match_move(info,SEMI, "do_while_stmt(): need SEMI"))
						    return parse_good_return(node, ok);
		}
...
static TreeNode * for_stmt(RDinfo * info, Bool *  ok){
	...
	if (match_move(info,FOR, "fo_stmt(): need FOR"))
	  if (match_move(info,LPAR, "for_stmt(): need LPAR"))
...
/* 21 Can return NULL
   ...
   If thisTokenNode is ; or ), returns a good-return NULL, do not consume the ; or 0 token.
...
static TreeNode * null_stmt(RDinfo * info, Bool *  ok){
	...
	if (match_move(info,SEMI, "null_stmt(): need SEMI"))
		return parse_good_return(node, ok);
...
static TreeNode * expr_stmt(RDinfo * info, Bool *  ok){
	...
	if (node->child[0]=expression(info, &status), status)
	  if (match_move(info,SEMI, "expr_stmt(): need SEMI"))
		  return parse_good_return(node, ok);
...
When succeed, returns
- a single node returned by assignment_expr(), if after calling assignment_expr() thisTokenNode is not COMMA.
...
static TreeNode * comma_expr(RDinfo * info, Bool *  ok){
	...
	while (!(check(info->thisTokenNode, RPAR)
				|| check(info->thisTokenNode, RBR) 
				|| check(info->thisTokenNode, SEMI))) {
		...
		rightNode = assignment_expr(info, &status);
...
static TreeNode * lhs(RDinfo * info, Bool *  ok){
	...
	TokenNode *curNode = thisTokenNode;
	TokenNode *currentNode = reach_node(thisTokenNode, 1);

	if (check(currentNode, LBR)) {
		node = array_element(info, ok);
		if (*ok == TRUE) return parse_good_return(node, ok);
	}
	else if (check(thisTokenNode, ID)) {
		if (match_move(info,ID, "lhs(): need ID")) {
			node = new_expr_node(ID_EXPR, curNode->lineNum);
			node->attr.exprAttr.name  = 
				string_clone(getTokenStr(curNode));
...
/* 29
   ...
   - If thisTokenNode is (, child[0] is what is returned by expression().
- Otherwise, if thisTokenNode is ID, child[0] is an expression-node with expression-kind as id-expression, and the name in its expression-attribute is the
  string of the id.
...
static TreeNode * array_element(RDinfo * info, Bool *  ok){
	...
	if (match_move(info,ID, "array_element() : need ID"))
		if (match_move(info,LBR, "array_element() : need LBR"))
			if (node = expression(info, &status), status)
...
When succeed:
- returns a single node that is returned by relational_expr(), if after calling relational_expr(), thisTokenNode is not == or !=.
...
static TreeNode * equality_expr(RDinfo * info, Bool *  ok){
	...
	if (check(info->thisTokenNode, EQ) 
			|| check(info->thisTokenNode, NEQ)) {

		parentNode = eqop(info, ok);
		...
		if (node != NULL)
			node->child[1] = equality_expr(info, ok);
	}
...
/* 32
   ...
   When succeed:
- returns a single node that is returned by additive_expr(), if after calling additive_expr(), thisTokenNode is not one of the relational operators > < <= >=.
...
TreeNode * relational_expr(RDinfo * info, Bool *  ok){
	...
	if (check(info->thisTokenNode, LT) 
			|| check(info->thisTokenNode, LTE)
			|| check(info->thisTokenNode, GT)
			|| check(info->thisTokenNode, GTE)) {

		parentNode = relop(info, ok);
		...
		if (node != NULL)
			node->child[1] = relational_expr(info, ok);
	}
...
/* 34
   ...
   When succeed:
- returns a single node that is returned by multiplicative_expr(), if after calling multiplicative_expr(), thisTokenNode is not one of the additional-operators + or -.
...
TreeNode * additive_expr(RDinfo * info, Bool *  ok){
  ...
  if (check(info->thisTokenNode, PLUS) 
		  || check(info->thisTokenNode, MINUS)) {

	  parentNode = addop(info, ok);
	  ...
	  if (node != NULL)
		  node->child[1] = additive_expr(info, ok);
  }
...
/* 36
   ...
   When succeed:
   - returns a single node that is returned by primary_expr(), if after calling primary_expr(), thisTokenNode is not one of the multiplicative-operators * or /.
...
TreeNode * multiplicative_expr(RDinfo * info, Bool *  ok){
	...
	if (check(info->thisTokenNode, TIMES) 
			|| check(info->thisTokenNode, OVER)) {

		parentNode = mulop(info, ok);
		...
		if (node != NULL)
			node->child[1] = multiplicative_expr(info, ok);
	}
...
/*37
 ...
 The token type of thisTokenNode in info, is saved as the op field in its expression-attribute.
 The child fields of this node are not assigned by any value in this function.
 */
static TreeNode *  mulop(RDinfo * info, Bool *  ok){
	TokenNode * thisTokenNode = info->thisTokenNode;
...
/* 38
   ...
   When succeed: depending on thisTokenNode, this function returns :
...
TreeNode * primary_expr(RDinfo * info, Bool *  ok) {
	...
	if (check(thisTokenNode, ID)) {
		if (currentNode != NULL && check(currentNode, LBR)) {
			node = array_element(info, &status);
		}
		else if (currentNode != NULL && check(currentNode, LPAR)) {
			node = call(info, &status);
		}
		else {
			node = new_expr_node(ID_EXPR, thisTokenNode->lineNum);
			node->attr.exprAttr.name = string_clone(getTokenStr(thisTokenNode));
			status = TRUE;
	...
	else if (check(thisTokenNode, LPAR)) {
...
static TreeNode * call(RDinfo * info, Bool *  ok){
	...
	if (match_move(info,ID, "call() : need ID"))
		if (match_move(info,LPAR, "call(): need LPAR")) {

			node = new_expr_node(CALL_EXPR, curNode->lineNum);
			node->attr.exprAttr.name = string_clone(getTokenStr(curNode));
...
/* 40
   ...
   It can normally return NULL if thisTokenNode is ), corresponding to the EMPTY case. Note that the ending ) is not consumed by this function.
   */
static TreeNode * arg_list(RDinfo * info, Bool *  ok){
