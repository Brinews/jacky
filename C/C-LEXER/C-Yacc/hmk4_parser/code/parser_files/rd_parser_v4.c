...
static const char* strOfToken(TokenNode *tn)
{
	return tn->token->string;
}

#define parseDebug(x) do { \
	if(RD_PARSE_DEBUG){ \
...
static TreeNode*  array_dcl(RDinfo * info, Bool *  ok){
	...
	if (check(info->thisTokenNode, VOID) || check(info->thisTokenNode, INT)) {
		if (check(n1, ID) && check(n2,LBR) && check(n3, NUM) && check(n4, RBR) && check(n5, SEMI)) {
			...
			node->attr.dclAttr.type = (type == VOID ? VOID_TYPE : INT_TYPE);
			node->attr.dclAttr.size = atoi(strOfToken(n3));
			node->attr.dclAttr.name = string_clone(strOfToken(n1));
...
static TreeNode * fun_dcl(RDinfo * info, Bool *  ok){
	...
	if (type != END && match_move(info, ID, "fun-dcl: need 'ID")) {
			...
			node->attr.dclAttr.type = (type == VOID ? VOID_TYPE : INT_TYPE);
			node->attr.dclAttr.name = string_clone(strOfToken(id));
...
TreeNode * compound_stmt(RDinfo * info, Bool *  ok){
	...
	if (match_move(info, LCUR, "compound-stmt(): need LCUR"))
		if (left = local_dcl_list(info, &status), status)
...
TreeNode * else_part(RDinfo * info, Bool *  ok){
	...
	else return parse_good_return(NULL, ok);
}
...
static TreeNode * array_element(RDinfo * info, Bool *  ok){
	...
	if (check(info->thisTokenNode, ID)) {
		...
		if (match_move(info, LPAR, "array_element(): need LPAR")) 
			if (node = expression(info, &status), status)
				if (match_move(info, RPAR, "array_element(): need RPAR"))
					if (match_move(info, LBR, "array_element(): need LBR"))
...
TreeNode * primary_expr(RDinfo * info, Bool *  ok) {
	...
	if (check(info->thisTokenNode, ID)) {
			if (check(n1, LBR)) {
				...
				node->attr.exprAttr.name = string_clone(strOfToken(info->thisTokenNode));
				status = TRUE;
			...
			node->attr.exprAttr.val = atoi(strOfToken(info->thisTokenNode));
...
static TreeNode * call(RDinfo * info, Bool *  ok){
	...
	if (match_move(info, ID, "call() : need ID"))
		if (match_move(info, LPAR, "call(): need LPAR")) {
			...
			node->attr.exprAttr.name = string_clone(strOfToken(n0));
