...
void ALU(unsigned A,unsigned B,char ALUControl,unsigned *ALUresult,char *Zero)
{
	...
	else if(ALUControl==0x1)  //减
		*ALUresult=A-B;
	
	else if(ALUControl==0x2)  //条件判断
	{
		if(A<B)
			*ALUresult=1;
		else
	...
	else if(ALUControl==0x4)  //与
		*ALUresult=A&B;
	
	else if(ALUControl==0x5)  //或
		*ALUresult=A|B;
	
	else if(ALUControl==0x6)  //左移
		*ALUresult=(B)<<16;
	
	else if(ALUControl==0x7)   //与非
		*ALUresult=~A;
