...
public:
	...
	Cloud(string id,int x,int y,int s,double spd,string color) : MoveableObject(id,x,y,s,spd){this ->color = color;};
	bool outofBorder();
	bool nearCloud(int x,int y);
	void draw();
	void remove();
...
Ship::Ship(const Ship& ship) : MoveableObject(ship.ID,ship.destinationX,ship.destinationY,ship.size,ship.speed){

	money = ship.money;
	cargo_hold = ship.cargo_hold;
	cargo = (TradeGood *)new TradeGood[cargo_hold];
	for(int i = 0;i < cargo_hold;i++ ){
		cargo[i] = ship.cargo[i];
	}
}
...
bool Ship::upgrade()
{
	...
	TradeGood * tmp_cargo = new TradeGood[cargo_hold];
	for(int i = 0;i < cargo_hold - 5;i++){
		tmp_cargo[i] = cargo[i];
	}
	for(int i = cargo_hold - 5;i < cargo_hold;i++){
		tmp_cargo[i] = EMPTY;
	}
...
bool Cloud::outofBorder()
{
	return xpos == 0 || ypos == 0 || xpos == XSIZE || ypos == YSIZE;
}

bool Cloud::nearCloud(int x, int y)
{
	...
	//if(abs(x-xpos) < size && abs(y - (maxY+ypos)) < size)
	if(sqrt(pow(x-xpos,2.0) + pow(y - ypos,2.0))< size)
	{
		return true;
	}
	return false;
...
void Cloud::draw()
{
	...
	window.setpos(xpos, ypos-1.0/4*size);
	window.plot();

	window.symbolsize(size/2);
	window.setpos(xpos-1.0/4*size, ypos);
	window.plot();
}
void Cloud::remove()
{
...
Cloud makeCloud()
{
	...
	int pos  = rand() % YSIZE ;
	int size = rand() % 50 + 70;
	string color = rand() % 5 == 0 ? "black" : "white";

	Cloud cloud;
	if(flag){
		cloud = Cloud("cloud",0,pos,size,CLOUD_SPD,color);
		cloud.setDestination(pos,0);
	}else{
		cloud = Cloud("cloud",pos,YSIZE,size,CLOUD_SPD,color);
		cloud.setDestination(XSIZE,pos);
	}
	flag = !flag;
...
void display(City c, Ship s)
{
	...
	for(int i =1 ; i < TRADEGOOD_COUNT; i++)
	{
		...
		cout << setw(10) << "(U)pgrade" << setw(10) << 5000 << setw(10)  << endl;
}
...
void trade(City c, Ship &s)
{
	...
	while(tolower(key) != 'q' && key != ' ')
	{
		...
		if(tolower(key) == 'u'){  //upgrade ship
			if(!s.upgrade()){
				message = "You do not have enough money to upgrade ship!\n";
			}
		}
...
//	getline(cin,shipName);
	...
	makeClouds(clouds,NUM_CLOUDS);
	//Cloud cloud = Cloud("cloud",0,200,100,8.0);
	//cloud.setDestination(880,1000);
	...
	while(lastTerrain != LAND)
	{
		...
		for(int i = 0;i < NUM_CLOUDS;i++){
			...
			if(clouds[i].outofBorder()){
				clouds[i] = makeCloud();
			}
			clouds[i].draw();

			if(clouds[i].getColor() == "black" && 
					clouds[i].nearCloud(trader.getX(),trader.getY())){
				cout << "GAME OVER! hit by a storm" << endl;
				return 0;
			}
		}
