...
public:
	...
	Cloud(string name,int x,int y,int s,double spd, string color) 
	...
	/* when boat overlapped by black cloud, it will be over */
	bool cloudOverlap(int x,int y);
...
bool Cloud::cloudOverlap(int x, int y)
{

	/* the overlap algorithm is not so standard */
...
Cloud makeSingleCloud()
{
	...
	string color = rand() % 5 == 1 ? "black" : "white";

	static int rand_direction = 0;

	Cloud cloud;
	if(rand_direction){
		cloud = Cloud("cloud-1",0,pos+1,size,CLOUD_SPEED,color);
		cloud.setDestination(pos,0);
	}else{
		cloud = Cloud("cloud-2",pos-1,YSIZE,size,CLOUD_SPEED,color);
	...
	while(lastTerrain != LAND)
	{
		...
		for(int i = 0;i < CLOUDS_NUM;i++){
			...
			if(clouds[i].getColor() == "black" && 
					clouds[i].cloudOverlap(trader.getX(),trader.getY())){
				cout << "GAME OVER!!! Hit by a storm...." << endl;
				return 0;
			}

			// update cloud color by random, 20% time black
			clouds[i].setColor(rand()%5==1?"black":"white");
		}
