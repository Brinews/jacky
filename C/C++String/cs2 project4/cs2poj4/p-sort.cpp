...
{sort_cpp.count(__LINE__,"main");
    ...
    // Options container
    Options opts;sort_cpp.count(__LINE__);
    ...
    // Generate data
    vector<int> data;sort_cpp.count(__LINE__);
...
{sort_cpp.count(__LINE__,"generate_random_data");
    ...
    srand(static_cast<unsigned int>(seed));sort_cpp.count(__LINE__);


    // Put random values in vector
    for (vector<int>::size_type idx = 0; idx < vec.size(); ++idx)
    {
        if(mod) { vec[idx] = rand() % mod;sort_cpp.count(__LINE__);
 }
        else    { vec[idx] = rand();sort_cpp.count(__LINE__);
       }
    }
}
...
{sort_cpp.count(__LINE__,"output_data");
    ...
    const int cols  =  7;sort_cpp.count(__LINE__);

    const int width = 10;sort_cpp.count(__LINE__);
...
{sort_cpp.count(__LINE__,"process_command_line");
    ...
    for (int idx = 1; idx < argc; ++idx)
    {
        ...
        string opt(argv[idx]);sort_cpp.count(__LINE__);
