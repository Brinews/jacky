...
ghreply ngethostbyname(char *host,char *dns_server, int recursion_desired, int print_messages) 
{
	...
	int yes = 1;
	if(setsockopt(s ,SOL_SOCKET,SO_REUSEADDR|SO_BROADCAST, &yes, sizeof(int))==-1)
	{
		perror("setsockopt");
		return retval;
	}
	...
	for(i=0;i<ntohs(dns->add_count);i++)
	{
		...
		{
			addit[i].rdata = (char* )malloc(ntohs(addit[i].resource->data_len));
...
peer_rq* peerRequest(char *buf, int size, char *ip)
{
	...
	/* send peer request */
	sendto(sockfd, buf, size, 0, (struct sockaddr *)&servaddr, sizeof(servaddr));

	int i = sizeof(cliaddr);
	n = recvfrom(sockfd, buf, size, 0, (struct sockaddr *)&cliaddr, (socklen_t*)&i);
	buf[n] = 0;
