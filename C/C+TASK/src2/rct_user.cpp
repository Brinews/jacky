...
vector<rct_task *> rct_user::getTaskByName(string name)
{
	vector<rct_task *> ret;

	for (int i = 0; i < allProjects.size(); i++) {
		vector<rct_task *> p = allProjects[i]->findTaskByName(name);

		for (int j = 0; j < p.size(); j++)
			if (p[j]->isVisible(getId())) ret.push_back(p[j]);
...
vector<rct_task *> rct_user::getTaskByCreateDate(string date)
{
	vector<rct_task *> ret;

	for (int i = 0; i < allProjects.size(); i++) {
		vector<rct_task *> p = allProjects[i]->findTaskByCreateDate(date);
		...
		string type, string uid)
{
	rct_task * task;
	if (type == "private") {
		task = new rct_private_task(projId, getId(), taskname);
		task->assignCompletor(uid);
	}
	else if (type == "assign") {
		task = new rct_assigned_task(projId, getId(), taskname);
		task->assignCompletor(uid);
	}
	else {
		task = new rct_normal_task(projId, getId(), taskname);
	}
...
vector<rct_task *> rct_leader::getTaskList(string projId)
{
	vector<rct_task *> null;

	for (int i = 0; i < allProjects.size(); i++)
		if ("" == projId || allProjects[i]->getProjectId() == projId) {
			/* 先排序 */
			allProjects[i]->scheduleTaskByMember();

			vector<rct_task *> ret = allProjects[i]->getTaskList();

			for (int j = 0; j < ret.size(); j++)
				if (ret[j]->getLeaderId() == getId() || 
						ret[j]->isVisible(getId()))
					null.push_back(ret[j]);
		}
...
vector<rct_task *> rct_member::getTaskList(string projId)
{
	...
	for (int i = 0; i < allProjects.size(); i++)
		if (allProjects[i]->getProjectId() == projId) {
			...
			vector<rct_task *> ret = allProjects[i]->getTaskList();
			for (int j = 0; j < ret.size(); j++)
				if (ret[j]->isVisible(getId())) null.push_back(ret[j]);
		}
	...
	for (int i = 0; i < allProjects.size(); i++) {
		if (taskId.find(allProjects[i]->getProjectId(), 0) == 0) {
			...
			for (int j = 0; j < tList.size(); j++) {
				// sure the task is publish state and executable
				if (tList[j]->getTaskId() == taskId
						&& tList[j]->isPublished()
						&& tList[j]->isExecutable(getId())) {
					// find the task
					taskDoneNum++;
