...
StrategyGetBuffer(BufferAccessStrategy strategy, bool *lock_held)
{
	...
	*lock_held = true;
	...
	for (;;)
	{
		...
		current_buf_id = StrategyControl->nextVictimBuffer;
		...
		if (round_more == 1) /* begin count */
		{
			round_count++;

			if (round_count >= NBuffers-1) 
			{
				round_more = 2; /* round ends */
			}
		}
		...
		if (buf->refcount == 0)
		{
			...
			else
			{
				/* when find the first buf where usage_count = 0, 
				   ...
				   usage_time */
				if (round_more == 0)  /* find an available buf first */
				{
					round_more = 1;
					min_usage_time = buf->usage_time;
				}
				else if (round_more == 1) /* find available buf in next round */
				{
					if (buf->usage_time < min_usage_time) 
					{
						min_buf_id = current_buf_id;
						min_usage_time = buf->usage_time; /* update min time (FIFO) */
					}
				}
				else if (round_more == 2) /* round ends */
				{
					UnlockBufHdr(buf);
