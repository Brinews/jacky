...
void Display(void)
{
   ...
   glPushMatrix();
	 ...
	 glRotatef( 360.0*year_earth/365.0, 0.0, 1.0, 0.0 );
   	 //glutWireSphere(2.2, 10, 10 );
	 drawStars();
   glPopMatrix();
   ...
   glPushMatrix();						
   	 ...
   	 glRotatef( 360.0*year_venus/365.0, 0.0, 1.0, 0.0 );
	 glRotatef(-90, 1.0, 0.0, 0.0);
	 glBindTexture(GL_TEXTURE_2D, textureVenus);
	 glTranslatef(VENUS_ORBIT, 0.0, 0.0 );

	 glPushMatrix();						
	   glRotatef( 360.0*day_venus/24.0, 0.0, 0.0, 1.0 );
   ...
   glPushMatrix();						
     ...
     glRotatef( 360.0*year_mars/365.0, 0.0, 1.0, 0.0 );
	 glRotatef(-90, 1.0, 0.0, 0.0);
	 glBindTexture(GL_TEXTURE_2D, textureMars);
	 glTranslatef(MARS_ORBIT, 0.0, 0.0 );

	 glPushMatrix();						
	   glRotatef( 360.0*day_mars/24.0, 0.0, 0.0, 1.0 );
   ...
   glPushMatrix();						
   	   glRotatef(-15, 0.0, 0.0, 1.0);
	   glRotatef( 360.0*day_earth/24.0, 0.0, 0.0, 1.0 );
   ...
   // draw moon
   glPushMatrix();						
   glRotatef( -360.0*year_moon/365.0, 0.0, 0.0, 1.0 );
...
void init()
{
	...
	/* earth */
	glGenTextures(3, &textureEarth);
	glBindTexture(GL_TEXTURE_2D, textureEarth);

	img2 = TextureLoadBitmap("earth.bmp", &w, &h);
	gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, img2);
	...
	/* moon */
	glGenTextures(4, &textureMoon);
	glBindTexture(GL_TEXTURE_2D, textureMoon);

	img3 = TextureLoadBitmap("moon.bmp", &w, &h);
	gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, img3);
	...
	/* mars */
	glGenTextures(2, &textureMars);
	glBindTexture(GL_TEXTURE_2D, textureMars);

	img1 = TextureLoadBitmap("mars.bmp", &w, &h);
	gluBuild2DMipmaps(GL_TEXTURE_2D, 3, w, h, GL_RGB, GL_UNSIGNED_BYTE, img1);
	...
	img = TextureLoadBitmap("venus.bmp", &w, &h);
	...
	for (i = 0; i < MAX_STAR; i++) {
		star[i].pos[0] = 8.0*rand()/RAND_MAX-4.0;
		star[i].pos[1] = 8.0*rand()/RAND_MAX-4.0;
		star[i].pos[2] = 8.0*rand()/RAND_MAX-4.0;
	}
}
...
void Idle(void)
{
   ...
   if (selfR == GL_TRUE) {
	   ...
	   day_earth = day_earth - ((int)(day_earth/24))*24;
	   day_venus = day_venus - ((int)(day_venus/24))*24;
	   day_mars = day_mars - ((int)(day_mars/24))*24;
   }

   if (globalR == GL_TRUE) {
	   year_earth += incStep/24.0;
	   year_venus += incStep*EARTH_YEAR/VENUS_YEAR/24.0;
	   year_mars += incStep*EARTH_YEAR/MARS_YEAR/24.0;

	   year_earth = year_earth - ((int)(year_earth/365))*365;
	   year_moon = 12.0 * year_earth;
	   year_mars = year_mars - ((int)(year_mars/365))*365;
	   year_venus = year_venus - ((int)(year_venus/365))*365;
   }
