...
ModelView planet[6] = {DisplaySphere, DisplaySun, DisplayVenus,
 					DisplayMars, DisplayEarth, 
					DisplayMoon};

GLuint textureEarth = -1, textureMars = -1, textureVenus = -1,
		 textureMoon = -1, textureSaturn = -1;
   ...
   glPushMatrix();						
	 ...
	 glPushMatrix();						
		 ...
		 glBegin(GL_QUAD_STRIP);
		    ...
		    for(i=0; i <= 360; i++)
			{
				glVertex3f(sin(i*3.1416/180)*0.8, cos(i*3.1416/180)*0.8, 0 );
				glVertex3f(sin(i*3.1416/180)*1.0, cos(i*3.1416/180)*1.0, 0 );
			}
		 glEnd();

		 glRotatef( PI_DEGREE*day_saturn/DAY_HOUR, 0.0, 0.0, 1.0 );
		 glScalef(Scale, Scale, Scale);
		 gluSphere(saturn, SATURN_RADIUS, 10, 10);
...
{
	int i;
	// Clear the window color before Displaying is performed
...
void init()
{
	...
	LoadImageTexture(&textureSaturn, &saturn, "saturn.bmp");
	...
	for (i = 0; i < MAX_STAR; i++) {
		star[i].pos[0] = 5.0*rand()/RAND_MAX-2.5;
		star[i].pos[1] = 5.0*rand()/RAND_MAX-2.5;
		star[i].pos[2] = sqrt(3.1*3.1-star[i].pos[0]*star[i].pos[0]- star[i].pos[1]*star[i].pos[1]);
...
void Idle(void)
{
   ...
   if (rControl == GL_TRUE) {
	   ...
	   if (earth_incline > 15.0 || earth_incline < -15.0) earthStep = -earthStep;
   }
